...
class MyHttp {
  ...
  static Future<Map<String, dynamic>?> postMethodJson({
    required String url,
    Map<String, dynamic>? bodyParams,
    bool wantSnackBar = true,
    ...
    if (kDebugMode) print("URL:: $url");
    if (kDebugMode) print("TOKEN:: $headers");
    if (kDebugMode) print("bodyParams:: ${bodyParams ?? {}}");
    ...
    if (await CommonWidgets.internetConnectionCheckerMethod()) {
      try {
        http.Response? response = await http.post(
          Uri.parse(url),
          body: jsonEncode(bodyParams ?? {}), // ← raw JSON string
          headers: headers,
        );
        ...
        if (await CommonWidgets.responseCheckForPostMethod(
          ...
          checkResponse?.call(response.statusCode);
          // ← decode and return the map directly
          return jsonDecode(response.body) as Map<String, dynamic>;
        } else {
          checkResponse?.call(response.statusCode);
          if (kDebugMode) {
            print("ERROR::statusCode=${response.statusCode}: body=${response.body}");
          }
          return null;
        }
      } catch (e) {
  ...
  static Future<http.Response?> postJsonRawMethod({
    required String url,
    Map<String, dynamic>? bodyParams,
    bool wantSnackBar = true,
    void Function(int)? checkResponse,
    ...
    authorization = {
      ...
      // Important to specify JSON content type
    };
    ...
    if (await CommonWidgets.internetConnectionCheckerMethod()) {
      try {
        // Convert bodyParams to JSON
        String jsonBody = jsonEncode(bodyParams ?? {});

        http.Response? response = await http.post(
          Uri.parse(url),
          body: jsonBody, // Sending JSON body
          headers: authorization,
        );
